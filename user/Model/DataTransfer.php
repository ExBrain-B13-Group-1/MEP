...
class DataTransfer
{
    ...
    public function isDuplicatedForEnrollClass($student_id, $enrolled_class_id){
        try {
            $db = new DBConnection();
            $pdo = $db->connection();
            $sql = $pdo->prepare(
                "SELECT * FROM t_student_classes_enroll WHERE student_id = :student_id AND class_id = :class_id"
            );
            $sql->bindValue(':student_id', $student_id, PDO::PARAM_INT);
            $sql->bindValue(':class_id', $enrolled_class_id, PDO::PARAM_INT);
            $sql->execute();
