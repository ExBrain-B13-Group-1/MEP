...
function formatDays($days)
{
    ...
    $selectedDays = "";
    for ($i = 0; $i < strlen($binaryDays); $i++) {
        if ($binaryDays[$i] === '1') {
            $selectedDays .= $daysOfWeek[$i] . ",";
        }
    }
    $selectedDays = rtrim($selectedDays, ' , '); // Remove the trailing comma
    echo $selectedDays; // Output: Mon,Wed,Thu,Sat
}
                    ...
                    <nav class="flex px-5 py-3 text-gray-700 border border-gray-200 rounded-lg bg-gray-50 dark:bg-gray-800 dark:border-gray-700" aria-label="Breadcrumb">
                        <ol class="inline-flex items-center space-x-1 md:space-x-2 rtl:space-x-reverse">
                            <li class="inline-flex items-center">
                                <a href="./classlist.php" class="inline-flex items-center text-base font-medium text-gray-700 hover:text-blue-600 dark:text-gray-400 dark:hover:text-white">
